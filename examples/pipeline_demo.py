...
def demo_object_pipeline():
    ...
    try:
        ...
        pipeline_obj = mpmsub.Pipeline(
            [["cat", temp_file], ["grep", "hello"], ["wc", "-l"]]
        )

        job = (
            mpmsub.Job(cmd=pipeline_obj)
            .cpu(1)
            .memory("50M")
...
def demo_mixed_jobs():
    ...
    jobs = [
        ...
        mpmsub.pipeline(
            [["echo", "Pipeline output"], ["tr", "a-z", "A-Z"]],
            p=1,
            m="50M",
            id="simple_pipeline",
        ),
...
def demo_stdout_stderr_redirection():
    """Demonstrate stdout/stderr redirection to files."""
    print("\n" + "=" * 60)
    print("STDOUT/STDERR REDIRECTION DEMO")
    print("=" * 60)

    with tempfile.TemporaryDirectory() as tmpdir:
        stdout_file = os.path.join(tmpdir, "output.txt")
        pipeline_file = os.path.join(tmpdir, "pipeline.txt")

        # Create cluster with alternative API syntax (cpu/memory instead of p/m)
        p = mpmsub.cluster(cpu=2, memory="1G", verbose=False, progress_bar=False)

        # Add jobs with stdout redirection using alternative API
        jobs = [
            mpmsub.job(
            ...
            mpmsub.pipeline(
                ...
                cpus=1,
                memory="50M",
                id="pipeline_redirect",
                stdout=pipeline_file,
            ),
        ]

        p.jobs.extend(jobs)

        print("Added jobs with output redirection:")
        for job in jobs:
            print(f"  {job}")
        ...
        if os.path.exists(stdout_file):
            with open(stdout_file, "r") as f:
                print(f"\nstdout file content: '{f.read().strip()}'")

        if os.path.exists(pipeline_file):
            with open(pipeline_file, "r") as f:
                print(f"pipeline file content: '{f.read().strip()}'")

        # Show job results (will show redirection messages)
        print(f"\nJob results:")
        for job in p.completed_jobs:
            print(f"  {job.job_id}: {job.stdout}")

        print(f"\n✓ Completed stdout/stderr redirection demo")
...
def main():
    ...
    # Demo 6: stdout/stderr redirection
    demo_stdout_stderr_redirection()
    ...
    print("  ✓ CalVer versioning (2025.09.16)")
    ...
    print("  ✓ Mixed single commands and pipelines")
    print("  ✓ stdout/stderr redirection to files")
    print("  ✓ Robust API supporting both p/m and cpu/memory parameters")
    print("  ✓ Builder pattern with .stdout_to() and .stderr_to() methods")
